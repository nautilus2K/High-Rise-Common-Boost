	...
	if (GetConVarBool(g_ConVar_CB))
	{
		...
		else
		{
			if (IsPlayerAlive(client) && !GetEntProp(client, Prop_Send, "m_isIncapacitated"))
			{
				...
				{
					new Float:vecVelZombie[3];
					vecVelZombie[2] = g_fSpeed_Entity[client];
					vecVel[2] = vecVelZombie[2];
					vecVel[2] += RoundToFloor((vecVel[2]/2)*FLOAT_PI*2);
					vecVel[2] = Cosine(vecVel[2])*SEQ_VEL_SHOVED_BACKWARD*2;
					vecVel[2] *= (FLOAT_PI/2.5);
