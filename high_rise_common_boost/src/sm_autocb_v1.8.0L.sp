...
*	24/04/2022 Vector 1.8.0L - Add new convar: st_highrisecb.
*/
...
#define PLUGIN_VER "1.8.0"
#define MAXCLIENTS 32
#define SEQ_VEL_SHOVED_BACKWARD 154.481
#define DEFAULT_HCB_VEL 350.0

new Handle:g_ConVar_CB;
new Handle:g_ConVar_HCB;
...
public OnPluginStart()
{
	...
	g_ConVar_HCB = CreateConVar("st_highrisecb", "0", "Will make for you 'high-rise' auto-commonboost", FCVAR_NOTIFY | FCVAR_CHEAT);
...
{
	if (GetConVarBool(g_ConVar_HCB) && !GetConVarBool(g_ConVar_CB))
	{
		if (!g_bIsCB_RootKey[client])
		{
			if (buttons & IN_JUMP)
			{
				...
				vecVel[2] = vecVel[2]+RoundToFloor((vecVel[2]/2)*FLOAT_PI*2);
				vecVel[2] = Cosine(vecVel[2])*DEFAULT_HCB_VEL;
				vecVel[2] = (vecVel[2]*2)+FLOAT_PI*2;
				if (vecVel[2] < 0) vecVel[2] *= -1;
